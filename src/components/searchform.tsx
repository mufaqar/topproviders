  ...
  return (
    <>
      {loader ? <div className='fixed z-50 inset-0 !w-full bg-black/60 flex items-center flex-col justify-center'><div className="custom-loader"></div></div> :
        <form>
          <div className="relative flex items-center w-full serch_form bg-white rounded-[3px] shadow-[0_0_12px_0_rgba(0,0,0,0.26)]">
            <FaMagnifyingGlass className="absolute ml-3 text-xl text-[#6746C8]" />
            <input type="text" onKeyDown={handleKeyDown} placeholder="Enter Zip Code" maxLength={5} name="zip_code" value={zipcode} onChange={(e) => setzipcode(e.target.value)} className="placeholder:text-lg placeholder:font-bold appearance-none w-full py-3 pl-12 pr-8 outline-none rounded-[3px]" />
            <button className="px-6 py-4 text-base uppercase font-semibold text-[#4B4B4B] bg-[#FECE2F] border-[#FECE2F] rounded-[3px]" onClick={handleState}>Search</button>
          </div>
