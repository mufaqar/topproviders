...
export default function CitiesModule({ allProviders, myCity }: any) {
  const query = useParams();
  const type = query?.zipcode?.[0] || "internet";
  const city_code = query?.zipcode?.[2];
  const state = query?.zipcode?.[1];
  let C_State = (state as string)?.toUpperCase();

  const inputString = city_code || "";
  const parts = inputString?.split("-");
  const capitalizedWords: string[] = parts?.map(
    (word: string) => word.charAt(0).toUpperCase() + word.slice(1)
  );
  const city = capitalizedWords.join(" ");
  allProviders = allProviders?.filter((item: any) =>
    item?.providers_types?.some((i: any) => i.toLowerCase() === type)
  );
  const servicesTypes = allProviders?.map((item: any) => {
    return item.providers_service_types;
  });
  ...
  const cheepProviders = allProvidersCheep.sort((a: any, b: any) => {
    if (type === "tv") {
      return (
        a.services_info_tv_services_price - b.services_info_tv_services_price
      );
    } else if (type === "internet-tv") {
      return (
        a.services_info_internet_tv_bundles_price -
        b.services_info_internet_tv_bundles_price
      );
    } else {
  ...
  const FastProviders = allProvidersFast.sort((a: any, b: any) => {
    const speedA = parseInt(
      a.services_info_internet_services_speed.split("-")[1],
      10
    );
    const speedB = parseInt(
      b.services_info_internet_services_speed.split("-")[1],
      10
    );
  ...
  return (
    <>
      <PageHead
        title={
          type === "internet"
            ? `Best ${totalProviderCount} ${formatType(
                type
              )}  Service Providers in ${city}, ${C_State}  | For ${currentYear} `
            : type === "tv"
            ? `Best ${totalProviderCount} ${formatType(
                type
              )}  Service Providers in ${city}, ${C_State}  | For ${currentYear}`
        ...
        description={`Best ${totalProviderCount} ${formatType(
          type
        )} Service Providers in ${city}, ${C_State}.  ${allProviders
          ?.slice(0, 8)
          .map((item: any, idx: number) => `${idx + 1} ${item?.title}`)
          .join(", ")}`}
        url={`https://topproviders.net/${state}/${city_code}?type=${type}`}
        curl={`https://topproviders.net/${state}/${city_code}`}
      />
        ...
        <div className="container mx-auto px-4">
          <div>
            <h1 className="sm:text-5xl text-2xl font-bold text-center max-w-[850px] mx-auto capitalize leading-10">
              {formatType(type)} Service Providers in <br />
              <span className="text-[#ef9831] ">
        ...
        <div className="container mx-auto px-4">
          <div className="mb-10">
            <h2 className="text-2xl font-bold  capitalize leading-10">
              {formatType(type)} Service Providers in{" "}
              <span className="text-[#ef9831] ">
                {city}, <span className="uppercase">{state}</span>
              </span>
            </h2>
          ...
          <div className="grid gap-7">
            {allProviders?.map((item: any, idx: number) => {
              var summaryData = {
                ...
                price:
                  type === "internet"
                    ? item.services_info_internet_services_price
                    : type === "tv"
                    ? item.services_info_tv_services_price
                    : type === "internet-tv" &&
                      item.services_info_internet_tv_bundles_price,
                channels:
                  item.services_info_internet_tv_bundles_summary_channel,
                speed:
                  type === "internet"
                    ? item.services_info_internet_services_speed
                    : type === "tv"
                    ? item.services_info_tv_services_speed
                    : type === "internet-tv" &&
                      item.services_info_internet_tv_bundles_speed,
                summery:
                  type === "internet"
                    ? item.services_info_internet_services_features
                    : type === "tv"
                    ? item.services_info_tv_services_features
                    : type === "internet-tv" &&
                      item.services_info_internet_tv_bundles_features,
              };
          ...
          <div>
            <p className="text-sm font-[Roboto] mt-10">
              *DISCLAIMER: Availability vary by service address. not all offers
              available in all areas, pricing subject to change at any time.
              Additional taxes, fees, and terms may apply.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="">
            <h2 className="text-2xl font-bold">
              Overview of {formatType(type)} Service Providers in{" "}
              <span className="text-[#ef9831] ">
                {city}, <span className="uppercase">{state}</span>
              </span>
            </h2>
      ...
      <section className="my-16">
        <div className="container mx-auto px-4">
          <div className="mb-10">
            <h2 className="text-2xl font-bold  capitalize leading-10">
              Cheap {formatType(type)} Service Providers in{" "}
              <span className="text-[#ef9831] ">
                {city}, <span className="uppercase">{state}</span>
              </span>
            </h2>
            <p className="text-xl font-[Roboto] mt-5">
              Affordability is essential when choosing your {formatType(type)}{" "}
              Service Provider; in an age where staying connected is more
              crucial than ever, we bring you budget-friendly {formatType(type)}{" "}
              options that don't compromise on quality. Below are the cheap{" "}
              {formatType(type)} Service Providers in {city}, {C_State}.
            </p>
          </div>
      ...
      {type !== "internet-tv" && type !== "tv" && (
        <section className="my-16">
          <div className="container mx-auto px-4">
            <div className="mb-10">
              <h2 className="text-2xl font-bold  capitalize leading-10">
                Fast {formatType(type)} Service Providers In{" "}
                <span className="text-[#ef9831] ">
                  {city}, <span className="uppercase">{state}</span>
                </span>
              </h2>
              <p className="text-xl font-[Roboto] mt-5">
                ...
                {formatType(type)} Service Providers in {city}, {C_State}. These
                providers offer impressive download speeds that cater to the
                needs of heavy internet users, streamers, and online gamers.
              </p>
            </div>
      ...
      <section className="my-16">
        <div className="container mx-auto px-4">
          <div className="mb-10">
            <h2 className="text-2xl font-bold">
              Summary of {formatType(type)} Service Providers in{" "}
              <span className="text-[#ef9831] ">
                {city}, <span className="uppercase">{state}</span>
              </span>
            </h2>
              ...
              <div className="w-full h-auto shadow-xl border rounded-t-md rounded-b-md flex md:flex-col flex-row items-stretch">
                <div
                  className={`md:w-full min-w-[50px] grid ${
                    type === "internet-tv" ? "md:grid-cols-8" : "md:grid-cols-7"
                ...
                >
                  ...
                  <div className="md:border-r border-r-0 md:border-b-0 border-b grid justify-center md:p-5 p-2 md:h-auto h-[120px] items-center">
                    <div>
                      <h4 className="md:text-base text-xs text-center text-white mb-2">
                        {type === "tv" ? "Channels" : "Max Download Speed"}
                      </h4>
                    </div>
                  </div>
                ...
                <div className="flex md:flex-col flex-row w-full md:overflow-hidden overflow-x-scroll">
                  {allProviders?.map((item: any, idx: number) => {
                    var summaryData = {
                      ...
                      summery:
                        type === "internet"
                          ? item.services_info_internet_services_summary_features
                          : type === "tv"
                          ? item.services_info_tv_services_summary_features
                          : type === "internet-tv" &&
                            item.services_info_internet_tv_bundles_summary_features,

                      speed:
                        type === "internet"
                          ? item.services_info_internet_services_summary_speed
                          : type === "tv"
                          ? item.services_info_tv_services_summary_speed
                          : type === "internet-tv" &&
                            item.services_info_internet_tv_bundles_summary_speed,
                      channel:
                        item.services_info_internet_tv_bundles_summary_channel,
                      price:
                        type === "internet"
        ...
        <div className="container mx-auto px-4">
          <div className="mb-10">
            ...
            <p className="text-base">
              {city}, {C_State} is well-connected with a diverse range of{" "}
              {formatType(type)} connection types to its residents, each with
              with its own advantages and considerations. These connection types
              include{" "}
